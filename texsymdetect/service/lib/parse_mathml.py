...
misdetecting colored formulas as errors---anything that's set to 'white' in a paper would be
...
class Token:
    text: str
    " Unicode (not TeX) representation of this token, computed by parsing the formula with KaTeX. "
    ...
    " 'start' and 'end' are measured relative to the start of the formula."
    ...
    """
    List of TeX macros that were applied to the letter to style it in the formula. Note that this
...
@dataclass
class Node:
    " Node for a parse tree for an formula. "

    type_: NodeType
    " A tag describing the role of the node in the formula. "
    ...
    start: int
    " Offset of the first character this node was parsed from in the formula. "

    end: int
    " Offset of the last character this node was parsed from in the formula. "
    ...
    of an formula with an equality. Should only be set to true for nodes that
    ...
    def is_symbol(self) -> bool:
        """
        Whether this node should be considered a 'symbol', i.e., an entity in an formula that
